...
class ModFile:
    def __init__(self, path):
        ...
        if self.type != 16:
            raise Exception("%s is not a mod file" % path)
    ...
    def fields(self, value_type):
        return [value_type() for _ in range(0, self.int())]
...
mod = ModFile("D:/Launcher/Steam/steamapps/common/Kenshi/data/gamedata.base")
...
for _ in range(0, data["record_count"]):
    record = {
        "instance_count": mod.int(),
        "type": mod.record_type(),
        "id": mod.int(),
        "name": mod.string(),
        "string_id": mod.string(),
        "datatype": RECORD_DATATYPE[mod.int()],
        "fields": {
